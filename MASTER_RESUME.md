...
Windows/Linux server administration, Active Directory (enterprise-scale), Windows Server Network Load Balancing (NLB), Hyper-V/VMware virtualization, NAS/iSCSI storage, backup & disaster recovery, ITIL-aligned operations, capacity planning, performance tuning, patch management. MS SQL database administration and schema design (proficient) — designed and managed database schemas supporting Exchange mail servers and other business systems (2008–2012). Greenfield office/site IT infrastructure buildout (network, server/AD, end-user environment, vendor procurement, physical site coordination) — Kuala Lumpur APAC development office, supporting 100+ staff (2015–2017).
...
Exchange Online mailbox administration, Microsoft Teams, SharePoint, licensing management, security & compliance policies; automated onboarding/offboarding workflows; VDI/Terminal Services, VoIP systems.
...
- Participated in Sitecore's SOC 2 compliance audit: implemented and maintained technical controls (access, logging, policy) and provided evidence/documentation to auditors.
