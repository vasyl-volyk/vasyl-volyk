...
Windows/Linux server administration, Active Directory (enterprise-scale), Windows Server Network Load Balancing (NLB), Hyper-V/VMware virtualization, NAS/iSCSI storage, backup & disaster recovery, ITIL-aligned operations, capacity planning, performance tuning, patch management. Out-of-band server management & firmware/BIOS troubleshooting — Dell iDRAC, HPE iLO (proficient) — remote console access and hardware monitoring on physical servers, spanning the KL office build (2015–2017) and current role (2021–present); also proficient in firmware updates/rollbacks, BIOS-level configuration changes, and hardware failure diagnosis from BMC/iDRAC/iLO logs (Sitecore, ongoing/periodic, 2021–present); note: generic IPMI protocol and Redfish API specifically not separately confirmed — vendor BMC tools (iDRAC/iLO) only. MS SQL database administration and schema design (proficient) — designed and managed database schemas supporting Exchange mail servers and other business systems (2008–2012). Greenfield office/site IT infrastructure buildout (network, server/AD, end-user environment, vendor procurement, physical site coordination) — Kuala Lumpur APAC development office, supporting 100+ staff (2015–2017).
...
- Performed occasional/basic use of Dell iDRAC / HPE iLO for out-of-band remote console access and hardware monitoring on physical servers, along with proficient-level, ongoing/periodic firmware updates and rollbacks, BIOS-level configuration changes, and hardware failure diagnosis from BMC/iDRAC/iLO logs.
...
- Out-of-band server management confirmed as of Jul 2026: Dell iDRAC / HPE iLO, proficient level, including remote console/monitoring plus firmware updates/rollbacks, BIOS-level configuration, and hardware failure diagnosis from BMC logs (Sitecore, ongoing/periodic, 2021–present; also KL office build 2015–2017 for the console/monitoring portion). Generic IPMI protocol and Redfish API specifically not separately confirmed — only vendor tools (iDRAC/iLO) used. Bare-metal provisioning at scale (racking, datacenter-wide hardware rollout, fleet-level BMC automation/scripting) not confirmed — revisit if gained.
...
- **DPU / GPU / accelerator / cloud-scale server platform experience** — confirmed as of Jul 2026: NO experience. Existing hardware/server-platform experience is general x86 rack servers (Dell/HPE) with BMC/BIOS-level troubleshooting; no DPU (e.g., NVIDIA BlueField), GPU fleet, or specialized accelerator hardware exposure. Revisit if gained.
- **Microsoft ICM (Incident Case Management) or equivalent fleet-scale incident platform** — confirmed as of Jul 2026: no direct ICM experience; closest confirmed adjacent tool is ServiceNow (incident logging/tracking plus change and problem management), current role. Revisit if ICM or a comparable fleet-ops incident platform is used directly.
