...
Microsoft Purview (familiar) — data retrieval via SDK/API-based querying, current role (Sitecore, 2021–present); not confirmed: data source scanning/cataloging, classification/sensitivity labeling, or catalog/lineage ownership. SQL Server Management Studio (SSMS) (proficient) — regular hands-on administration of on-premises SQL Server instances, including backup operations and instance configuration; spans multiple roles, exact role/year breakdown not itemized **[NEEDS INPUT]** (likely overlaps with the MS SQL schema-design work below but not confirmed as the full extent). Azure SQL Managed Instance (proficient) — administration, current role (Sitecore, 2021–present); specific task scope (provisioning vs. ongoing admin vs. migration) not itemized **[NEEDS INPUT]**. MS SQL database administration and schema design (proficient) — designed and managed database schemas supporting Exchange mail servers and other business systems (2008–2012). Note: SSIS and SSRS — confirmed as of Sep 2026, no experience with either.
...
- Used Microsoft Purview (familiar level) for SDK/API-based data retrieval and querying as part of data platform operations.
- Administered SQL Server via SQL Server Management Studio (SSMS, proficient level) — backup operations and instance configuration for on-premises SQL Server instances — and administered Azure SQL Managed Instance (proficient level) as part of the current role's database footprint.
- Built automated workflows using Azure Logic Apps (proficient level) covering provisioning/reporting automation, notifications/alerting, and integration between systems.
...
- **Azure pub/sub messaging (Service Bus, Event Grid, Event Hub)** — confirmed as of Sep 2026: NO experience.
...
- **Microsoft Purview — confirmed as of Sep 2026:** familiar level — SDK/API-based data retrieval and querying, current role (Sitecore, 2021–present). Not confirmed: data source scanning/cataloging, classification/sensitivity labeling, or catalog/lineage ownership — these remain open if a posting weighs them heavily. Revisit if deeper Purview governance work is gained.
- **SSMS (SQL Server Management Studio) — confirmed as of Sep 2026:** proficient level — regular backup operations and instance configuration for on-premises SQL Server instances, spans multiple roles; exact role/year breakdown not itemized **[NEEDS INPUT]**.
- **Azure SQL Managed Instance — confirmed as of Sep 2026:** proficient level — administration, current role (Sitecore, 2021–present). Specific task scope (provisioning vs. ongoing admin vs. migration) not itemized **[NEEDS INPUT]**.
- **SSIS, SSRS — confirmed as of Sep 2026:** NO experience with either.
- **Azure Logic Apps — confirmed as of Sep 2026:** proficient level — built automated workflows covering a mix of provisioning/reporting automation, notifications/alerting, and system integration, current role (Sitecore, 2021–present).
- **Azure API Management — confirmed as of Sep 2026:** NO substantive hands-on experience — flagged during tailoring but no concrete usage, context, or timeframe could be confirmed. Treat as an honest gap, not a resume item.
- **Azure Service Bus, Event Grid, Event Hubs — confirmed as of Sep 2026:** NO experience with any of these pub/sub messaging services.
