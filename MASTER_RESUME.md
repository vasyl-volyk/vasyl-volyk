...
Large-scale Azure multitenant environments (5 tenants, 1,700 users, 500 servers); cross-tenant application deployment, automation, and Azure AD / Entra ID group & user syncing; VNets, NSGs, VPN/ExpressRoute, RBAC, Conditional Access. On-premises-to-Azure migration — led a full-scale lift-and-shift migration of on-premises VMware virtual machine infrastructure to Azure using Azure Migrate, covering AD/domain services, file servers, web servers, CRM, SQL Server databases, and development/test (CI/CD) environments, delivered under an aggressive/limited migration timeline (Sitecore, Senior Infrastructure & Cloud Engineer role). Microsoft Fabric (exposure/light use) — light, occasional hands-on use in current role to store and manage group, user, and approval data backing a self-service Power App portal. Hands-on AWS (familiar) — EC2, S3, and IAM policy configuration, used in a supporting capacity alongside the primary Azure environment. GCP (exposure) — hands-on labs (Compute, IAM, etc.) completed as part of a DevOps training course; no production experience.
...
### VMware-to-Azure Lift-and-Shift Migration
- Led/executed a full-scale migration of on-premises VMware virtual machine infrastructure to Azure using Azure Migrate, as part of the Sitecore Senior Infrastructure & Cloud Engineer role.
- Scope covered AD/domain services, file servers, web servers, CRM, SQL Server databases, and development/test (CI/CD) environments — a broad, mixed workload migration rather than a single-app rehost.
- Delivered under an aggressive/limited migration timeline **[NEEDS INPUT: exact duration/deadline if available]**.
- Outcome: qualitative — full workload set successfully rehosted to Azure; no formal metrics (downtime, cost delta, timeline-vs-target) captured yet — add if they become available.
...
### Self-Service Access Portal — Microsoft Fabric + Power App
- Used Microsoft Fabric (light/occasional hands-on use) to store and manage group, user, and approval data backing a self-service access-request Power App portal, enabling business users to submit and track requests.
...
- Led a full-scale lift-and-shift migration of on-premises VMware virtual machine infrastructure to Azure using Azure Migrate, covering AD/domain services, file servers, web servers, CRM, SQL Server databases, and development/test (CI/CD) environments, delivered under an aggressive/limited migration timeline.
...
- Used Microsoft Fabric (light/occasional hands-on use) to store and manage group, user, and approval data backing a self-service access-request Power App portal for business users.
