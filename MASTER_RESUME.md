...
- Optional future metrics (left qualitative for now by choice): Terraform governance provisioning (counts of management groups/subscriptions/policies/role assignments) and Bicep VM provisioning (number of VMs, time saved per build). Documentation as Code metric captured (~100 pages migrated). AWS captured at familiar level (EC2, S3, IAM) — add scale/impact if it grows. Azure FinOps/cost governance captured (expert level, tagging/invoice sections/owner ID/idle resource cleanup) — add hard $ or % savings figure if it becomes available. Hub-spoke/Private Endpoints/Private DNS scale (number of VNets, endpoints, or zones) — add if it becomes available; currently qualitative only. On-call rotation captured at participant level (3-person rotation, ~weekly incidents) — add MTTR/noise-reduction figures if they become available, and note if role evolves to include designing the rotation/escalation policy itself.
- Open gaps surfaced during tailoring (no experience confirmed yet): Kubernetes/EKS/AKS (confirmed as of Jul 2026 — exposure only, via hands-on labs in the same DevOps training course as the GCP labs; no production experience), GitOps/ArgoCD/Helm/Atlantis/Terragrunt (confirmed as of Jul 2026 — no experience), Go (confirmed as of Jul 2026 — no experience), Kafka/streaming systems (confirmed as of Jul 2026 — no experience), Datadog (confirmed as of Jul 2026 — no experience; existing observability stack is Grafana/Prometheus/Zabbix/SolarWinds/LogicMonitor plus Azure Monitor/Sentinel), API gateway / service mesh technologies — Kong, Istio, Envoy, NGINX, Traefik, Azure API Management (confirmed as of Jul 2026 — no experience), ClickHouse/Druid or other analytics/time-series databases (confirmed as of Jul 2026 — no experience), PostgreSQL, Redis (confirmed as of Jul 2026 — no experience), Puppet, CloudFormation, Ansible (confirmed — Terraform/Bicep only), ELK/OpenSearch stack (confirmed — no experience), container security tools — Falco, OPA/Gatekeeper, image scanning, runtime protection (confirmed — no experience), AWS Security Specialist/OSCP/CKS/GCPN/CISSP certs (confirmed — none held), Azure Data Factory, Kubecost/OpenCost, CUR-based pipelines, Karpenter, Graviton, AWS Savings Plans/RIs, GitHub Actions, PAM, SOAR, executive technical/financial roadmap presentation. OpenTelemetry/distributed tracing (confirmed as of Jul 2026 — no experience; existing tracing-adjacent work is Wireshark packet capture and Application Insights APM, not OpenTelemetry specifically). Chaos engineering/fault-injection testing (confirmed as of Jul 2026 — no experience). Formal production-readiness reviews partnering with software teams pre-launch (confirmed as of Jul 2026 — no experience; adjacent CI/CD deployment support exists but not a formal PRR process). Web3/crypto-native engineering (confirmed as of Jul 2026 — no experience). Formal SLO/SLI/error-budget frameworks (confirmed as of Jul 2026 — no experience; existing practice is threshold-based alerting and RCA via Sentinel/Log Analytics for security and Grafana/Prometheus/Zabbix for proactive infra monitoring, not formal SLO definitions). 24/7/365 formal on-call rotation beyond the 3-person weekly rotation (confirmed as of Jul 2026 — no additional experience beyond that rotation).
- Bash: confirmed as of Jul 2026 — familiar level, used for Linux/Unix server administration scripting, alongside legacy Shell/Perl from early-career roles.
